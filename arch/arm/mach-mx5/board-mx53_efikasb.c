...
static const struct imxuart_platform_data mx53_efikasb_uart_data __initconst = {
	.flags = IMXUART_HAVE_RTSCTS,
};

static struct ipuv3_fb_platform_data efikasb_fb_data[] = {
	{
	.disp_dev = "lcd",
	.interface_pix_fmt = IPU_PIX_FMT_BGR24,
	.mode_str = "WSVGA",
				   ...
				   char **cmdline, struct meminfo *mi)
{
	...
	if (mem_tag) {
		...
		efikasb_fb_data[0].res_base[0] = gpu_data.reserved_mem_base
					+ gpu_data.reserved_mem_size;
		efikasb_fb_data[0].res_size[0] = fb_mem;
	}
}
...
static void __init mx53_efikasb_board_init(void)
{
	...
	imx53_add_imx_uart(0, &mx53_efikasb_uart_data);
