...
static inline void sppp_pwr_cache_vbat_update(const uint8_t *vbat)
{
	...
	printk(KERN_ERR "%s %x %x", __func__,
	       sppp_cache.vbat[0], sppp_cache.vbat[1]);
}
...
static inline void sppp_pwr_cache_stat_update(const sppp_rx_t *resp)
{
	...
	printk(KERN_ERR "%s STATUS data: is GET? %d; %x %x %x %x",
...
{
	printk(KERN_ERR "%s id: %d", __func__, packet->id);
...
{
	printk(KERN_ERR "%s %d", __func__, mv);
...
static void sppp_pwr_set_current_accum(void)
{
	printk(KERN_ERR "%s not implemented", __func__);
}
...
sppp_pwr_update_status_voltage(struct sppp_pwr_device_info *di)
{
	...
	printk(KERN_ERR "%s mV: %d, uV: %d",
	       __func__, di->voltage_raw, di->voltage_uV);

	calculate_capacity_percentage(di->voltage_raw);
...
static void sppp_pwr_write_status(struct sppp_pwr_device_info *di,
                                  char status)
{
...
static void sppp_pwr_write_active_full(struct sppp_pwr_device_info *di,
                                       int active_full)
{
	printk(KERN_ERR "%s not implemented", __func__);
}
...
static void sppp_pwr_monitor_work(struct work_struct *work)
{
	...
	printk(KERN_ERR "%s Sending VBAT, STATUS request", __func__);
	sppp_request_vbat();
	sppp_request_status();
